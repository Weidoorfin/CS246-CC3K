...
import { Config, loadServerHierarchicalMemory, setGeminiMdFilename as setServerGeminiMdFilename, getCurrentGeminiMdFilename, ApprovalMode, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_EMBEDDING_MODEL, FileDiscoveryService, MCPServerConfig, IDE_SERVER_NAME, } from '@google/gemini-cli-core';
...
export async function loadCliConfig(settings, extensions, sessionId, argv) {
    ...
    if (ideMode) {
        if (mcpServers[IDE_SERVER_NAME]) {
            logger.warn(`Ignoring user-defined MCP server config for "${IDE_SERVER_NAME}" as it is a reserved name.`);
        }
        const companionPort = process.env.GEMINI_CLI_IDE_SERVER_PORT;
        if (companionPort) {
            const httpUrl = `http://localhost:${companionPort}/mcp`;
            mcpServers[IDE_SERVER_NAME] = new MCPServerConfig(undefined, // command
            undefined, // args
            undefined, // env
            undefined, // cwd
            undefined, // url
            ...
            logger.warn('Could not connect to IDE. Make sure you have the companion VS Code extension installed from the marketplace or via /ide install.');
        }
    }
